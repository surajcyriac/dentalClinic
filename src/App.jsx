    ...
    <div id="home" className="min-h-screen bg-[#f4f4f4]  scroll-smooth ">
      ...
      <header className="bg-white shadow-md sticky top-0 z-50">
        <div className="max-w-full px-4 sm:px-6 lg:px-12">
          <div className="flex justify-between items-center py-4">
            <div>
              <h1 className="md:text-4xl  sm:text-[15rem]  font-bold text-[#2B2B25]">Dr. Nithin's Dental Care</h1>
              <p className="text-sm text-[#2B2B25]">Excellence in Dental Healthcare</p>
            </div>

            {/* Desktop Nav */}
            <nav className="hidden md:flex md:space-x-4 lg:space-x-8">
              <a href="#home" className="font-semibold text-[#2B2B25] hover:text-[#5F5F55] transition-colors md:text-[.90rem] lg:text-[1rem]" >Home</a>
              <a href="#experts" className="font-semibold text-[#2B2B25]0 hover:text-[#5F5F55] transition md:text-[.90rem] lg:text-[1rem]">Our Experts</a>
              <a href="#clinics" className="font-semibold text-[#2B2B25] hover:text-[#5F5F55] transition md:text-[.90rem] lg:text-[1rem]">Clinics</a>
              <a href="#services" className="font-semibold text-[#2B2B25] hover:text-[#5F5F55] transition md:text-[.90rem] lg:text-[1rem]">Services</a>
              <a href="#contact" className="font-semibold text-[#2B2B25] hover:text-[#5F5F55] transition md:text-[.90rem] lg:text-[1rem]">Contact</a>
          ...
          {mobileMenuOpen && (
            <nav className="md:hidden pb-4 space-y-2">
              <a href="#home" className="block text-[#2B2B25] hover:text-[#5F5F55] transition py-2  ">Home</a>
              <a href="#experts" className="block text-[#2B2B25] hover:text-[#5F5F55] transition py-2">Our Experts</a>
              <a href="#clinics" className="block text-[#2B2B25] hover:text-[#5F5F55] transition py-2">Clinics</a>
              <a href="#services" className="block text-[#2B2B25] hover:text-[#5F5F55] transition py-2">Services</a>
              <a href="#contact" className="block text-[#2B2B25] hover:text-[#5F5F55] transition py-2">Contact</a>
            </nav>
          )}
        </div>
      </header>
      ...
      <section className="relative bg-gradient-to-r from-[#5F5F55] to-[#2B2B25] text-white py-20 scroll-smooth">
        <div className="max-w-full px-4 sm:px-6 lg:px-12 text-right py-5">
          ...
          <p className="font-semibold text-xl  text-teal-50">Comprehensive dental care across two premium locations</p>
          <p className="font-semibold text-xl mb-12 text-teal-50">Comprehensive dental care</p>
          <a href="#clinics" className="mt-5 inline-flex items-center bg-white text-[#5F5F55] px-8 py-3 rounded-full font-semibold hover:bg-[#e3e3e0] transition">
        ...
        <div className="max-w-full px-4 sm:px-6 lg:px-12 ">
          ...
          <div className="flex flex-col md:flex-row items-center gap-20 max-w-6xl mx-auto ">
            ...
            <div className="flex-1 text-center md:text-left bg-gray-100 px-12 py-8 rounded-2xl shadow-lg">
              <h2 className="md:text-2xl sm:text-4xl font-bold  text-[#2B2B25] mb-1 leading-10 ">Meet Our Doctor, <br></br><span className='text-[#5F5F55] text-5xl' > Nithin Neeps</span></h2>
              <h3 className="text-lg font-bold text-[#464545] mb-4">Chief Dental Surgeon</h3>
...
/>
      ...
      <section id="experts" className="pt-30 pb-16 bg-gray-50">
        <div className="max-w-full px-4 sm:px-6 lg:px-12">
          <h2 className="text-3xl sm:text-4xl font-bold text-center text-[#2B2B25] mb-4">Our Experts</h2>
            ...
            {experts.map((expert, idx) => (
              ...
              >
                ...
                <div className="flex-1 text-center md:text-left">
                  <h3 className="text-2xl sm:text-3xl font-bold text-[#5F5F55] mb-2">{expert.name}</h3>
                  <p className="text-xl text-[#2B2B25] font-semibold mb-2">{expert.specialization}</p>
                  <p className="text-sm text-[#2B2B25] mb-1">{expert.qualification}</p>
                  <p className="text-sm text-[#2B2B25] font-semibold mb-4">{expert.experience} of Experience</p>
                  <p className="text-[#666666] leading-relaxed">{expert.description}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6  ">
            {services.map((service, idx) => (
              <div key={idx} className="bg-white p-6  rounded-xl border border-[#2B2B25] hover:border-[#5F5F55] hover:shadow-2xl transition-all duration-300 group">
                ...
                <p className="text-sm text-[#2B2B25] leading-relaxed group-hover:scale-105 transition-transform duration-300">{service.description}</p>
              </div>
            ))}
          </div>
        </div>
...
/></div>
          <div className="flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-6">
            ...
            <a href="tel:+919876543211" className="bg-white text-[#2B2B25] px-8 py-3 rounded-full font-semibold hover:bg-[#e3e3e0] transition">
